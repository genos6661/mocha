...
function loadData(reset = false) {
  ...
  $.ajax({
    url: url_api + `/master-report/partner?${params.toString()}`,
    type: 'GET',
    headers: {
      "Authorization": `Bearer ${window.token}`,
      "X-Client-Domain": myDomain
    ...
    success: function (response) {
      ...
      $('#totalPartner').text(response.total_count);

      if (details.length === 0) {
        if (offset === 0) {
          tbody.append('<tr><td colspan="8" class="text-center">Partner Data Not Found</td></tr>');
        }
        hasMoreData = false;
...
$('#sbmFilter').click(function (e) {
  ...
  const baseUrl = $('#urlToGo').val() || 'partner-report';
